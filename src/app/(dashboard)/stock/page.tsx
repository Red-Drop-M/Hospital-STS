...
export default function Stock() {
  ...
  const fetchBloodBags = async () => {
      try {
          ...
          if (response.data) {
              console.log("Setting blood bags data:", response.data.bloodBags);
              setPaginatedData(response.data.bloodBags);
  ...
  const handleCreate = async (values: BloodBagDTO) => {
      const response = await createBloodBag(values);
      if (response.data) {
        // Rafraîchir la liste
        fetchBloodBags();
      }
  ...
  if (error) {
      ...
      const handleSubmit = async (values: z.infer<typeof createBloodBagSchema>) => {
          try {
              // Créer l'objet BloodBagDTO avec les champs requis
              const newBloodBag: BloodBagDTO = {
                  id: crypto.randomUUID(), // Génère un UUID pour l'ID
                  BloodType: values.BloodGroup,
                  BloodBagType: values.BloodBagType,
                  BloodBagStatus: "aquired", // Status initial
              ...
              if (response.data) {
                  fetchBloodBags(); // Rafraîchir la liste
  ...
  return (
      <main className="w-full space-y-10 p-4 md:p-8">
          ...
          <Card className="mt-3 hover:border-red-900 transition-colors duration-300">
              ...
              <CardContent>
                  <div className="w-full mt-4">                      <GenericTable
                          columns={bloodBagColumns(setPaginatedData, setIsUpdateModalOpen, setSelectedBloodBag)}
                          data={paginatedData || []}
          ...
          {selectedBloodBag && (
            <GenericForm
              ...
              onSubmit={async (values) => {
                try {
                  const response = await updateBloodBag(selectedBloodBag.id, values);
                  if (response.data) {
                    fetchBloodBags();
                    setIsUpdateModalOpen(false);
                    toast({
                      title: "Succès",
                      description: "Poche de sang mise à jour avec succès",
                    });
                  }
                } catch (error) {
                  ...
                  toast({
                    title: "Erreur",
                    description: "Échec de la mise à jour de la poche de sang",
                    variant: "destructive",
                  });
                }
              }}
              submitButtonText="Mettre à jour"
              defaultValues={selectedBloodBag}
            />
          )}
        </DialogContent>
      </Dialog>
